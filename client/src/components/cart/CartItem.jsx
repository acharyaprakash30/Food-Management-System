...
const CartItem = ({ item }) => {
  ...
  const handleIncrement = async () => {
    ...
    try {
      const isIncreased = await dispatch(
        incrementQuantity({ userId, itemId })
      ).unwrap();
      const fetchUpdatedCart = await dispatch(
        getAllCartItems(userData?.id)
      ).unwrap();
  ...
  const handleDecrement = async () => {
    ...
    try {
      const isIncreased = await dispatch(
        decrementQuantity({ userId, itemId })
      ).unwrap();
      const fetchUpdatedCart = await dispatch(
        getAllCartItems(userData?.id)
      ).unwrap();
    ...
    <motion.li className="w-full flex items-center justify-start bg-zinc-800 rounded-md drop-shadow-md px-2 sm:pl-2 xs:pr-3 sm:pr-4 gap-4">
      ...
      <div className="flex items-center justify-start gap-1 w-full h-20 ">
        <h3 className="text-[15px] xs:text-base md:text-lg text-primary font-semibold h-auto   w-28 xs:w-40 ">
          <span className="line-clamp-2 font-sans">{item.name}</span>
        ...
        <div className="ml-auto flex items-center justify-center gap-3">
          <motion.div
            className="h-7 w-7 sm:w-8 sm:h-8 flex items-center justify-center rounded-md drop-shadow-md bg-zinc-950 cursor-pointer active:bg-green-600"
            {...btnClick}
            onClick={handleIncrement}
          >
            <p className="text-xl sm:text-xl font-semibold text-primary ">+</p>
          </motion.div>
          <p className="text-base sm:text-lg text-primary font-semibold flex">
            x{item.quantity}
          </p>
          <motion.div
            className="w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center rounded-md drop-shadow-md bg-zinc-950 cursor-pointer active:bg-blue-600"
            {...btnClick}
