...
const createOrder = async (req, res) => {
  ...
  try {
    ...
    const createOrderItems = await models.orderItems.bulkCreate(
      orderItemsToCreate,
      { transaction }
    ); //passing transaction
...
const getUserAllOrder = async (req, res) => {
  try {
    const checkRole = await models.users.findOne({
      where: { id: req.params.userId },
    });
    if (checkRole.role === "admin") {
      const userOrders = await models.orders.findAll({
        include: [{ model: models.items }, { model: models.users }],
        order: [
            ['id', 'DESC']
        ],
      });
...
const updateOrderedStatus = async (req, res) => {
  try {
    const userOrders = await models.orders.update(
...
module.exports = {
  ...
  updateOrderedStatus: updateOrderedStatus,
};
